![OpenCode SEO cover: an OpenCode command palette with /seo-audit, schema, geo, content, and backlinks commands over a dark CRT panel](assets/cover.svg)

# OpenCode SEO: SEO Tool for OpenCode

**OpenCode SEO is an open-source SEO analysis tool for [OpenCode](https://opencode.ai).** It runs 25 sub-skills and 18 specialist agents in parallel across technical SEO, content quality (E-E-A-T), Schema.org markup, AI search optimization (GEO), local SEO, e-commerce, and international SEO. Every audit produces a prioritized action plan with testable recommendations grounded in primary-source guidance from Google.

> **This is a port of [Claude SEO](https://github.com/AgriciDaniel/claude-seo) by [Agrici Daniel](https://agricidaniel.com/about).** All credit for the SEO methodology, sub-skills, agents, Python scripts, and analysis framework goes to AgriciDaniel and the [community contributors](CONTRIBUTORS.md). This fork adapts the tool for [OpenCode](https://opencode.ai) while preserving every feature, script, and analysis capability of the original.

[![CI](https://github.com/DevShaded/claude-seo/actions/workflows/ci.yml/badge.svg)](https://github.com/DevShaded/claude-seo/actions/workflows/ci.yml)
[![OpenCode](https://img.shields.io/badge/OpenCode-Tool-blue)](https://opencode.ai)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Version](https://img.shields.io/github/v/release/DevShaded/claude-seo)](https://github.com/DevShaded/claude-seo/releases)
...
> Using Claude Code instead of OpenCode? The original [Claude SEO](https://github.com/AgriciDaniel/claude-seo) tool is the source project — install it directly for Claude Code support.
...
![OpenCode SEO command demo](screenshots/seo-command-demo.gif)

Run a full audit and watch parallel agents fan out across the site:

![OpenCode SEO audit demo: parallel subagents producing a prioritized action plan](screenshots/seo-audit-demo.gif)
...
```bash
curl -fsSL https://raw.githubusercontent.com/DevShaded/claude-seo/main/install.sh > install.sh
...
> **Why `git clone` instead of `irm | iex`?** Downloading and executing remote code without verification is a supply chain risk. The `git clone` approach lets you inspect `install.ps1` before running it.
...
![OpenCode SEO sub-skill ecosystem: 25 modules grouped into categories around the central orchestrator](assets/sub-skills.svg)
...
OpenCode SEO writes real markdown reports as its primary deliverable. Below is the first ~50 lines of a `/seo-schema https://rankenstein.pro/about` audit verbatim. The actual structure, headers, and grading format the tool produces follows.
...
![OpenCode SEO audit signal flow: /seo-audit enters the orchestrator, fans out to 25 sub-skills and 6 parallel audit agents, and converges through the scoring engine into a prioritized report](assets/signal-flow.svg)

Commands are loaded from `commands/seo-*.md`, agents from `agents/seo-*.md`, and supporting skill reference material from `skills/seo-*/`. The orchestrator (`commands/seo.md`) handles industry detection (SaaS, local, ecommerce, publisher, agency), parallel sub-agent dispatch up to 15 simultaneously, and synthesis through the [10-principle framework](#methodology) before emitting the action plan. Full architecture: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).

## Methodology

![OpenCode SEO 10-principle methodology: PERCEIVE, ANALYZE, VALIDATE, and ACT phases with 10 principles arranged by quadrant](assets/framework.svg)
...
```bash
curl -fsSL https://raw.githubusercontent.com/DevShaded/claude-seo/main/uninstall.sh | bash
```
...
Optional MCP servers add live data to the audit pipeline. OpenCode SEO ships extensions for 8 servers; the tool core works without any of them.
...
OpenCode SEO is adapted from a family of interoperable tools:

| Tool | What it does | How it connects |
|-------|-------------|-----------------|
| [Claude SEO](https://github.com/AgriciDaniel/claude-seo) | SEO analysis, audits, schema, GEO (Claude Code) | Original source project. |
...
| [Codex SEO](https://github.com/AgriciDaniel/codex-seo) | Codex-first SEO skill suite | Port. Same SEO system adapted for Codex. |
...
4. `/seo-content-brief "target keyword"`: generate SEO-optimized content brief

## Documentation

- [Installation Guide](docs/INSTALLATION.md)
- [Commands Reference](docs/COMMANDS.md): every `/seo-` command in depth
- [Architecture](docs/ARCHITECTURE.md): agent-based design, parallel dispatch
- [Migration v1 to v2](docs/MIGRATION-v1-to-v2.md): breaking changes, six phases of work
- [Troubleshooting](docs/TROUBLESHOOTING.md)
- [Contributors](CONTRIBUTORS.md): community credits

## FAQ

### What is OpenCode SEO?

OpenCode SEO is a port of [Claude SEO](https://github.com/AgriciDaniel/claude-seo) adapted for [OpenCode](https://opencode.ai). It runs 25 sub-skills and 18 specialist agents in parallel across technical SEO, content quality, Schema.org markup, AI search optimization, local SEO, e-commerce, and international SEO. Audits produce a prioritized action plan where each recommendation carries the first-principle observation it rests on, its dependency relationship to other recommendations, a "how would we know this failed?" check, and a leading indicator. The tool is MIT-licensed, ships zero proprietary tracking, and works fully offline if you skip the optional Google API and MCP-extension enrichments. Aligned with [Google's AI Optimization Guide](https://developers.google.com/search/docs/fundamentals/ai-optimization-guide) and the September 2025 Quality Rater Guidelines.

### How is OpenCode SEO different from Screaming Frog or Ahrefs Site Audit?

Different surface area, different tradeoffs. **Screaming Frog** crawls deeper and faster at the link-graph level; it is purpose-built as a crawler and OpenCode SEO does not attempt to replace it. **Ahrefs Site Audit** brings a proprietary backlink index and link intelligence; OpenCode SEO integrates with Ahrefs via its MCP extension rather than competing. Where OpenCode SEO leads: conversational LLM-native workflow, recommendation falsifiability (every finding carries an explicit failure-mode check), open-source MIT licensing with zero per-domain pricing, AI search optimization aligned with Google's primary-source guidance, and primary-source schema-deprecation tracking. Use Screaming Frog or Ahrefs for what they are best at; use OpenCode SEO when you want LLM-driven synthesis, conversational iteration, and AI-search-first audits.

### Does OpenCode SEO work on single-page applications (Next.js, React, Vue)?

Yes. It ships a shared headless renderer (`scripts/render_page.py`) backed by Playwright Chromium. Every fetching subagent supports `--render auto` (the default), which auto-detects SPA hallmarks (empty `<div id="root">` shells, single bundle script, hydration markers) and switches to a rendered fetch. Set `--render always` to force rendering, or `--render never` for raw HTML only. Content extraction uses [trafilatura](https://github.com/adbar/trafilatura) for boilerplate removal. Publication dates come from [htmldate](https://github.com/adbar/htmldate). Known nuance: pages with scroll-bound hydration or post-interaction content fetches still produce noisy findings; see the [Limitations](#limitations) section for the recommended `seo-visual` cross-check workflow on those edge cases.

### What Google APIs does OpenCode SEO use, and are they required?

None are required. OpenCode SEO is fully functional with zero API keys. A 4-tier credential system lets you upgrade gradually: Tier 0 (API key only) unlocks PageSpeed Insights, CrUX, and CrUX History (25-week trend data). Tier 1 (+ OAuth or service account) adds Search Console with queries, URL Inspection, sitemap status, and the Indexing API. Tier 2 (+ GA4 property config) adds organic traffic, top landing pages, and device / country breakdowns. Tier 3 (+ Ads developer token) adds Keyword Planner search volume and competition data. The credential setup wizard runs via `/seo-google setup`. All credentials live under `~/.config/claude-seo/` with `0o600` file permissions; nothing is checked into the repo and nothing is transmitted beyond Google's own endpoints.

### Is OpenCode SEO free?

Yes. MIT licensed, fully open source, no per-domain pricing, no telemetry, no API quotas imposed by the tool itself. The core tool and all 25 sub-skills work without any paid service. Some optional MCP extensions wrap paid services (DataForSEO, Ahrefs, Profound, SE Ranking) where you bring your own account credentials; their use is opt-in and the tool works fully without them. Google APIs (PageSpeed Insights, Search Console, Indexing, GA4) are free from Google with normal account quota limits and require your own credentials.

### How is OpenCode SEO different from regular SEO tools when it comes to AI search?

Most SEO tools treat AI search as a separate optimization discipline. OpenCode SEO follows [Google's own position](https://developers.google.com/search/docs/fundamentals/ai-optimization-guide) that AEO and GEO are rebranded labels for SEO. AI Overviews and AI Mode are grounded in the same ranking systems as classic Search; the eligibility floor is normal indexation. OpenCode SEO scores passage citability (134-167 word self-contained answer blocks), question-based heading hierarchy, attribution density, and entity presence across Wikipedia, Reddit, YouTube, and LinkedIn. It explicitly rejects three influencer myths: llms.txt as a citation lever, content chunking for AI, and AI-specific keyword rewriting. For commerce sites, OpenCode SEO audits the IPTC `TrainedAlgorithmicMedia` requirement on AI-generated product images per Google Merchant Center policy.
...
**Original Claude SEO** created by **[Agrici Daniel](https://agricidaniel.com/about)**, AI Workflow Architect, and open-sourced at [AgriciDaniel/claude-seo](https://github.com/AgriciDaniel/claude-seo). All credit for the SEO methodology, the 10-principle framework, the sub-skill and agent system, the 50 Python execution scripts, the Schema.org templates, and the analysis pipelines goes to AgriciDaniel and the [community contributors](CONTRIBUTORS.md).

This project is a port of Claude SEO to [OpenCode](https://opencode.ai), re-adapting the command interface, agent definitions, and configuration format for the OpenCode harness while preserving the complete feature set.

- [AgriciDaniel.com](https://agricidaniel.com/about)
- [Blog](https://agricidaniel.com/blog): deep dives on AI marketing automation
- [AI Marketing Hub (free)](https://www.skool.com/ai-marketing-hub): open community
- [AI Marketing Hub Pro](https://www.skool.com/ai-marketing-hub-pro): Pro community, early access
...
Contributions welcome. Please read [CONTRIBUTING.md](CONTRIBUTING.md) before submitting PRs. The project enforces a per-PR audit gate covering manifest consistency, the test suite, and security review before any merge to `main`.
