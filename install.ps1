...
try {
    ...
    Write-Host "[DONE] $pythonVersion detected" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "[DONE] Git detected" -ForegroundColor Green
} catch {
...
$RepoTag = if ($env:CLAUDE_SEO_TAG) { $env:CLAUDE_SEO_TAG } else { 'v1.4.0' }
...
try {
    ...
    if (Test-Path $reqFile) {
        try {
            ...
            Write-Host "  [X]  Could not auto-install Python packages." -ForegroundColor Yellow
            Write-Host "  Try: $($python.Exe) $($python.Args -join ' ') -m pip install -r `"$installedReqFile`"" -ForegroundColor Yellow
        }
    } else {
        Write-Host "  [X]  No requirements.txt found; skipping Python dependency install." -ForegroundColor Yellow
    }
    ...
    try {
        ...
        Write-Host "  [X]  Playwright install failed. Visual analysis will use WebFetch fallback." -ForegroundColor Yellow
    }
} catch {
    Write-Host ""
...
Write-Host ""
Write-Host "[DONE] Claude SEO installed successfully!" -ForegroundColor Green
Write-Host ""
