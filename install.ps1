...
try {
    ...
    if (Test-Path $reqFile) {
        try {
            ...
            Write-Host "  WARNING: Could not auto-install Python packages." -ForegroundColor Yellow
            Write-Host ('  Try: {0} {1} -m pip install -r "{2}"' -f $python.Exe, ($python.Args -join ' '), $installedReqFile) -ForegroundColor Yellow
        }
    } else {
        Write-Host "  WARNING: No requirements.txt found; skipping Python dependency install." -ForegroundColor Yellow
    }
    ...
    try {
        ...
        Write-Host "  WARNING: Playwright install failed. Visual analysis will use WebFetch fallback." -ForegroundColor Yellow
    }
} catch {
    Write-Host ""
