...
main() {
    ...
    removed_commands=0
    for cmd_path in "${COMMANDS_DIR}"/seo*.md; do
        if [ -f "${cmd_path}" ]; then
            rm -f "${cmd_path}"
    ...
    if [ "${removed_skills}" -eq 0 ] && [ "${removed_agents}" -eq 0 ] && [ "${removed_commands}" -eq 0 ]; then
    ...
    echo "✓ Claude SEO uninstalled (${removed_skills} skill dirs, ${removed_agents} agent files, ${removed_commands} command files)."
}
